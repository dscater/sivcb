...
const { oTipoIngreso, limpiarTipoIngreso } = useTipoIngresos();
const accion = ref(props.accion_dialog);
const dialog = ref(props.open_dialog);
let form = useForm(oTipoIngreso.value);
...
watch(
    ...
    async (newValue) => {
        ...
        if (dialog.value) {
            ...
            if (oTipoIngreso.value.acceso == 1) {
            ...
            form = useForm(oTipoIngreso.value);
        }
    }
);
watch(
...
const enviarFormulario = () => {
    let url =
        form["_method"] == "POST"
            ? route("tipo_ingresos.store")
            : route("tipo_ingresos.update", form.id);
