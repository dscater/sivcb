...
let form = useForm(oUnidadMedida.value);
...
watch(
    ...
    async (newValue) => {
        ...
        if (dialog.value) {
            ...
            if (oUnidadMedida.value.acceso == 1) {
            ...
            form = useForm(oUnidadMedida.value);
        }
    }
);
watch(
...
const enviarFormulario = () => {
    let url =
        form["_method"] == "POST"
            ? route("unidad_medidas.store")
            : route("unidad_medidas.update", form.id);
