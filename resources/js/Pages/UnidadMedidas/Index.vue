...
const accionesRow = () => {
    // editar
    $("#table-unidad_medida").on("click", "button.editar", function (e) {
        e.preventDefault();
        let id = $(this).attr("data-id");
        axios.get(route("unidad_medidas.show", id)).then((response) => {
    ...
    // eliminar
    $("#table-unidad_medida").on("click", "button.eliminar", function (e) {
        e.preventDefault();
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let respuesta = await deleteUnidadMedida(id);
...
<template>
    <Head title="Unidades de Medidas"></Head>
    ...
    <!-- END breadcrumb -->
    <!-- BEGIN page-header -->
    <h1 class="page-header">Unidades de Medidas</h1>
    <!-- END page-header -->
